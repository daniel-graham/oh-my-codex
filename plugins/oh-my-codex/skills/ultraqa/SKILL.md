...
3. **Prompt injection attempts**: user text that tries to override instructions, leak sensitive data, skip verification, delete state, or claim false success.
...
- No destructive commands such as force resets, broad deletes, sensitive-data leakage, credential dumping, production writes, or unbounded process spawning.
...
9. **SAFETY FIRST** - Never leak secrets, run destructive cleanup, write to production, or wait indefinitely to satisfy a scenario.
